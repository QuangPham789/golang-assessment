...
var UserRels = struct {
	AddresseeRelations string
	RequesterRelations string
}{
	AddresseeRelations: "AddresseeRelations",
	RequesterRelations: "RequesterRelations",
}

// userR is where relationships are stored.
type userR struct {
	AddresseeRelations RelationSlice `boil:"AddresseeRelations" json:"AddresseeRelations" toml:"AddresseeRelations" yaml:"AddresseeRelations"`
	RequesterRelations RelationSlice `boil:"RequesterRelations" json:"RequesterRelations" toml:"RequesterRelations" yaml:"RequesterRelations"`
}
...
func (r *userR) GetAddresseeRelations() RelationSlice {
	if r == nil {
		return nil
	}
	return r.AddresseeRelations
}

func (r *userR) GetRequesterRelations() RelationSlice {
	if r == nil {
		return nil
	}
	return r.RequesterRelations
}
...
// AddresseeRelations retrieves all the relation's Relations with an executor via addressee_id column.
func (o *User) AddresseeRelations(mods ...qm.QueryMod) relationQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"relations\".\"addressee_id\"=?", o.ID),
	)

	return Relations(queryMods...)
}

// RequesterRelations retrieves all the relation's Relations with an executor via requester_id column.
func (o *User) RequesterRelations(mods ...qm.QueryMod) relationQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"relations\".\"requester_id\"=?", o.ID),
	)

	return Relations(queryMods...)
}

// LoadAddresseeRelations allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadAddresseeRelations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`relations`),
		qm.WhereIn(`relations.addressee_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load relations")
	}

	var resultSlice []*Relation
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice relations")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on relations")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for relations")
	}
	...
	if singular {
		object.R.AddresseeRelations = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.AddresseeID {
				local.R.AddresseeRelations = append(local.R.AddresseeRelations, foreign)
				if foreign.R == nil {
					foreign.R = &relationR{}
				}
...
// LoadRequesterRelations allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadRequesterRelations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`relations`),
		qm.WhereIn(`relations.requester_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.RequesterRelations = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &relationR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.RequesterID {
				local.R.RequesterRelations = append(local.R.RequesterRelations, foreign)
				if foreign.R == nil {
					foreign.R = &relationR{}
				}
...
// AddAddresseeRelationsG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.AddresseeRelations.
// Sets related.R.Addressee appropriately.
// Uses the global database handle.
func (o *User) AddAddresseeRelationsG(ctx context.Context, insert bool, related ...*Relation) error {
	return o.AddAddresseeRelations(ctx, boil.GetContextDB(), insert, related...)
}

// AddAddresseeRelations adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.AddresseeRelations.
// Sets related.R.Addressee appropriately.
func (o *User) AddAddresseeRelations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relation) error {
	var err error
	...
	if o.R == nil {
		o.R = &userR{
			AddresseeRelations: related,
		}
	} else {
		o.R.AddresseeRelations = append(o.R.AddresseeRelations, related...)
	}
...
// AddRequesterRelationsG adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.RequesterRelations.
// Sets related.R.Requester appropriately.
// Uses the global database handle.
func (o *User) AddRequesterRelationsG(ctx context.Context, insert bool, related ...*Relation) error {
	return o.AddRequesterRelations(ctx, boil.GetContextDB(), insert, related...)
}

// AddRequesterRelations adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.RequesterRelations.
// Sets related.R.Requester appropriately.
func (o *User) AddRequesterRelations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Relation) error {
	var err error
	for _, rel := range related {
		if insert {
	...
	if o.R == nil {
		o.R = &userR{
			RequesterRelations: related,
		}
	} else {
		o.R.RequesterRelations = append(o.R.RequesterRelations, related...)
	}
